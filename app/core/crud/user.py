...
from app.core.models import User, UserTransaction
...
async def delete_user(
        session: AsyncSession, user_id: int, delete_by: UserSchema
    ):
    """Deletes user from DB by specified ID and returns its object"""
    user_schema = await session.scalar(
        select(User).options(joinedload(User.role))
        .options(
            joinedload(User.created_transactions)
            .joinedload(UserTransaction.type_name)
        )
    ...
    elif (user_schema.role.position == "Owner" and
        delete_by.role.position != "Owner"):
        await session.close()
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                f"User with role «{delete_by.role.position}» "
